...
type LoadResult =
  | { error: string }
  | { showFull: boolean; tabsData: Record<MarketTab, DayLine[]> };

async function loadData(showFull: boolean): Promise<LoadResult> {
  try {
...
export default async function Last14DaysPage({
  ...
  const result = await loadData(full === "1");
  ...
  return (
    <div>
      <PageHeading
        emoji="📅"
        title={result.showFull ? "Tout l'historique" : "14 derniers jours"}
        subtitle="Le tableau NIVA — 1 ligne par jour, cumul du net"
        right={
          <Link
            href={result.showFull ? "/14-jours" : "/14-jours?full=1"}
          ...
          >
            {result.showFull ? "→ 14 jours" : "→ tout l'historique"}
          </Link>
        }
      />
      <DayTableBoard tabsData={result.tabsData} showTrend={!result.showFull} />
    </div>
  );
}
