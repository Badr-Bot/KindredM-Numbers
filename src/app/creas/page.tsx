import { getDataMode, HISTORY_START, referenceToday } from "@/lib/data";
...
type LoadResult = { error: string } | { creas: CreasData; today: string };

async function loadData(): Promise<LoadResult> {
  try {
    const today = await referenceToday();
    const creas = await getCreasData(HISTORY_START, today);
    return { creas, today };
  } catch (err) {
...
export default async function CreasPage() {
  ...
  return (
    <div>
      ...
      />
      <CreasBoard creas={result.creas} today={result.today} historyStart={HISTORY_START} />
    </div>
  );
}
